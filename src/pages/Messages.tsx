...
import { Send, Search, ArrowLeft, Mail } from "lucide-react";
      ...
      <main className="flex-1 lg:ml-0 mt-14 lg:mt-0">
        <div className="h-screen flex flex-col max-w-6xl mx-auto">
          ...
          <div className="flex-1 flex overflow-hidden">
            {/* Conversations List */}
            <div className={`${conversationId ? "hidden md:flex" : "flex"} w-full md:w-80 border-r border-border flex-col bg-card`}>
              ...
              <div className="flex-1 overflow-y-auto">
                {searchQuery ? (
                  ...
                  <div className="p-8 text-center text-muted-foreground">
                    <p>No messages yet</p>
                    <p className="text-xs mt-2">Search for followers to start chatting</p>
                  </div>
                ) : (
                  <div className="p-2">
                    {conversations.map((conv) => (
                      <div
                        key={conv.userId}
                        className={`flex items-center gap-3 p-3 hover:bg-muted rounded-xl cursor-pointer transition-colors mb-1 ${
            ...
            {/* Messages Area */}
            <div className={`${conversationId ? "flex" : "hidden md:flex"} flex-1 flex-col bg-background`}>
              {conversationId && selectedUser ? (
                <>
                    ...
                    {messages.map((msg) => (
                      ...
                      >
                        ...
                        >
                          ...
                          <p className={`text-[10px] mt-0.5 ${msg.senderId === user?.uid ? "text-primary-foreground/60" : "text-muted-foreground"}`}>
                  ...
                  <div className="border-t border-border p-3 bg-card">
                    <div className="flex gap-2">
                      ...
                      />
                      <Button onClick={handleSendMessage} size="icon" className="rounded-full" disabled={!newMessage.trim()}>
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground p-8">
                  <Mail className="h-20 w-20 mb-4 opacity-20" />
                  <p className="text-lg font-semibold">Your Messages</p>
                  <p className="text-sm text-center mt-2">Send private messages to a follower</p>
                </div>
              )}
            </div>
          </div>
