...
const CommentDialog = ({ postId, isOpen, onClose }: CommentDialogProps) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-screen h-screen max-w-none max-h-none m-0 rounded-xl md:rounded-3xl p-0 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-2">
          {comments.length === 0 ? (
            <p className="text-center text-muted-foreground py-12">No comments yet. Be the first to comment!</p>
          ) : (
              ...
              {comments.map((comment) => {
                ...
                return (
                  <div key={comment.id} className="space-y-2">
                    {/* Show parent comment for replies */}
                    {parentComment && (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          <div className="flex items-center gap-2 min-w-0 flex-wrap">
                            <span className="font-semibold text-sm truncate">{comment.displayName}</span>
                            <span className="text-muted-foreground text-xs truncate">@{comment.username}</span>
                          </div>
